...
const merchantLogoURL=d=>{try{const u=new URL(d?.source||'');const host=u.hostname.replace(/^www\\./,'');return host?'https://www.google.com/s2/favicons?domain='+encodeURIComponent(host)+'&sz=512':''}catch{return ''}};
...
function cardImage(d){const dealImage=safeURL(d.image),logo=merchantLogoURL(d);if(dealImage)return `<img class="card-art" src="${escapeHTML(dealImage)}" data-logo-fallback="${escapeHTML(logo)}" alt="Official artwork for ${escapeHTML(shortTitle(d))}" loading="lazy" referrerpolicy="no-referrer">`;if(logo)return `<img class="card-logo" src="${escapeHTML(logo)}" data-logo-only="1" alt="${escapeHTML(d.merchant||shortTitle(d))} logo" loading="lazy" referrerpolicy="no-referrer">`;return fallback(d)}
function render(){results=filterDeals();const names=[...selectedCategories];$('results-title').textContent=names.length===0?'The latest finds':names.length===1?names[0]:`${names.length} categories`;$('result-count').textContent=`${results.length} ${results.length===1?'find':'finds'}${selectedAreas.size?` · ${selectedAreas.size} ${selectedAreas.size===1?'location':'locations'}`:' across Singapore'}`;const active=$('search').value||selectedAreas.size||selectedCategories.size;$('active-filters').innerHTML=active?'<button class="clear-inline" id="clear-inline">Clear all filters ×</button>':'';$('clear-inline')?.addEventListener('click',reset);$('empty').hidden=results.length>0;$('more').hidden=results.length<=limit;$('grid').innerHTML=results.slice(0,limit).map(d=>{const s=status(d);return `<article class="card"><div class="card-image">${cardImage(d)}<span class="category-tag">${escapeHTML(d.category)}</span></div><div class="card-body"><div class="card-topline"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M19 10c0 5-7 11-7 11S5 15 5 10a7 7 0 1 1 14 0Z"/><circle cx="12" cy="10" r="2.5"/></svg>${escapeHTML(d.area)}</div><h3><button data-deal="${escapeHTML(d.id)}">${escapeHTML(shortTitle(d))}</button></h3><p class="summary">${escapeHTML(d.summary)}</p><div class="card-bottom"><span class="expiry${s.soon?' soon':''}">${escapeHTML(s.text)}</span><button class="view" data-deal="${escapeHTML(d.id)}" aria-label="View details: ${escapeHTML(shortTitle(d))}">View deal <span aria-hidden="true">↗</span></button></div></div></article>`}).join('');$('grid').querySelectorAll('img').forEach(img=>{const replaceWithFallback=()=>{const card=img.closest('.card');const id=card?.querySelector('[data-deal]')?.dataset.deal;const d=deals.find(x=>x.id===id);img.outerHTML=fallback(d||{category:'Deal details below'})};img.addEventListener('load',()=>{const w=img.naturalWidth||0,h=img.naturalHeight||0;if(img.dataset.logoOnly==='1'){if(w<96||h<48)replaceWithFallback();return}if(Math.max(w,h)<400||Math.min(w,h)<180){const logo=img.dataset.logoFallback;if(logo){img.removeAttribute('data-logo-fallback');img.dataset.logoOnly='1';img.className='card-logo';img.src=logo;img.alt='Merchant logo'}else replaceWithFallback()}},{once:true});img.addEventListener('error',()=>{const logo=img.dataset.logoFallback;if(logo&&img.src!==logo){img.removeAttribute('data-logo-fallback');img.dataset.logoOnly='1';img.className='card-logo';img.src=logo;img.alt='Merchant logo';return}replaceWithFallback()},{once:true})})}
function reset(){selectedCategories.clear();selectedAreas.clear();$('search').value='';limit=12;drawCategories();drawAreas();render()}
function openDeal(id){const d=deals.find(x=>x.id===id);if(!d||!isActive(d))return;const members=Array.isArray(d.members)?d.members:[];const memberHTML=members.length?`<div class="group-members"><h3>${members.length} offers in this group</h3>${members.map(m=>`<article class="group-member"><strong>${escapeHTML(m.title||'Deal')}</strong>${m.offer?`<p>${escapeHTML(m.offer)}</p>`:''}${m.location?`<span>📍 ${escapeHTML(m.location)}</span>`:''}${safeURL(m.source)?`<a href="${escapeHTML(safeURL(m.source))}" target="_blank" rel="noopener noreferrer">Official offer ↗</a>`:''}</article>`).join('')}</div>`:'';const detailImage=safeURL(d.image);const logo=merchantLogoURL(d);$('detail-content').innerHTML=`${detailImage?`<img class="detail-image" src="${escapeHTML(detailImage)}" data-logo-fallback="${escapeHTML(logo)}" alt="Official offer artwork" referrerpolicy="no-referrer">`:logo?`<div class="detail-logo-wrap"><img class="detail-logo" src="${escapeHTML(logo)}" alt="${escapeHTML(d.merchant||shortTitle(d))} logo" referrerpolicy="no-referrer"></div>`:''}<div class="detail-body"><p class="eyebrow">${escapeHTML(d.category)} · ${escapeHTML(d.area)}</p><h2 id="detail-title">${escapeHTML(shortTitle(d))}</h2><div class="detail-caption">${escapeHTML(d.caption)}</div>${memberHTML}<div class="detail-links">${safeURL(d.source)?`<a href="${escapeHTML(safeURL(d.source))}" target="_blank" rel="noopener noreferrer">Official details ↗</a>`:''}${safeURL(d.telegram)?`<a class="secondary" href="${escapeHTML(safeURL(d.telegram))}" target="_blank" rel="noopener noreferrer">View Telegram post ↗</a>`:''}</div></div>`;const detail=$('detail-content').querySelector('.detail-image');detail?.addEventListener('error',()=>{if(logo){detail.outerHTML=`<div class="detail-logo-wrap"><img class="detail-logo" src="${escapeHTML(logo)}" alt="Merchant logo" referrerpolicy="no-referrer"></div>`}else detail.remove()},{once:true});$('detail').showModal()}
